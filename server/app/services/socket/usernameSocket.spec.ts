...
import { UsernameSocket } from "./UsernameSocket.service";

/* tslint:disable:no-magic-numbers */

describe("UsernameSocket", () => {
  const usernameSocket: UsernameSocket = new UsernameSocket();

  // isEmpty
  it("Should be true if input empty", () => {
    const emptyString: string = "";
    expect(usernameSocket.isEmpty(emptyString)).to.equal(true);
  });
  it("Should be false if input is not empty", () => {
    const notEmptyString: string = "notEmpty";
    expect(usernameSocket.isEmpty(notEmptyString)).to.equal(false);
  });

  // isCorrectLength
  it("Should be false if input length is under 3", () => {
    const notEmptyString: string = "12";
    expect(usernameSocket.isCorrectLength(notEmptyString)).to.equal(false);
  });
  ...
  it("Should be true if input length is between 3 and 12", () => {
    ...
    expect(usernameSocket.isCorrectLength(notEmptyString)).to.equal(true);
  });

  // isAlpha
  it("Should be true if input is alpha", () => {
    const notEmptyString: string = "a";
    expect(usernameSocket.isAlpha(notEmptyString)).to.equal(true);
  });
  it("Should be false if input is numeric", () => {
    const notEmptyString: string = "6";
    expect(usernameSocket.isAlpha(notEmptyString)).to.equal(false);
  });
  ...
  it("Should be false if input is alpha", () => {
    const notEmptyString: string = "a";
    expect(usernameSocket.isNumeric(notEmptyString)).to.equal(false);
  });
  it("Should be true if input is numeric", () => {
    const notEmptyString: string = "6";
    expect(usernameSocket.isNumeric(notEmptyString)).to.equal(true);
  });
  it("Should be false if input is not alpha or numeric", () => {
    const notEmptyString: string = "%";
    expect(usernameSocket.isNumeric(notEmptyString)).to.equal(false);
  ...
  it("Should be true if input contains only alpha", () => {
    ...
    expect(usernameSocket.isAlphaNumeric(notEmptyString)).to.equal(true);
  });
  ...
  it("Should be false if input contains unsupported symbols", () => {
    ...
    expect(usernameSocket.isAlphaNumeric(notEmptyString)).to.equal(false);
  });
  ...
  it("Should be true if the array is empty", () => {
    ...
    expect(usernameSocket.isArrayEmpty(emptyArray)).to.equal(true);
  });
  it("Should be false if the array is not empty", () => {
    const notEmptyArray: string[] = ["Jack o' Lantern"];
    expect(usernameSocket.isArrayEmpty(notEmptyArray)).to.equal(false);
  });
  it("Should be false if the array is not empty", () => {
  ...
  it("Should be false if the array is not empty after add function", () => {
    ...
    usernameSocket.add("username", notEmptyArray);
    expect(usernameSocket.isArrayEmpty(notEmptyArray)).to.equal(false);
  });
  ...
  it("Should be true if the length of the array corresponds to the number of added username", () => {
    const notEmptyArray: string[] = [];
    usernameSocket.add("username1", notEmptyArray);
    usernameSocket.add("username2", notEmptyArray);
    usernameSocket.add("username3", notEmptyArray);
  ...
  it("Should be true if the array is empty", () => {
    const emptyArray: string[] = [];
    expect(usernameSocket.isAvailable("username", emptyArray)).to.equal(true);
  });
  it("Should be true if the username is not in the array", () => {
    ...
    usernameSocket.add("username2", notEmptyArray);
    usernameSocket.add("username3", notEmptyArray);
    expect(usernameSocket.isAvailable("username4", notEmptyArray)).to.equal(true);
  });
  it("Should be false if the username is in the array", () => {
    const notEmptyArray: string[] = [];
    usernameSocket.add("username", notEmptyArray);
    expect(usernameSocket.isAvailable("username", notEmptyArray)).to.equal(false);
  });
  it("Should be false if the username is in the array and the array has multiple usernames", () => {
    ...
    usernameSocket.add("username2", notEmptyArray);
    usernameSocket.add("username3", notEmptyArray);
    expect(usernameSocket.isAvailable("username3", notEmptyArray)).to.equal(false);
  });
  // verifyUsername
  ...
  it("Should be true if the username is empty", () => {
    ...
    usernameSocket.verifyUsername(mockReq(request), mockRes(response));
    expect(usernameSocket.usernameArray[0]).to.equal(user);
    usernameSocket.deleteUsername(mockReq(request));
  });
  it("Should be true if the username (ALPHA) is corretly added when the array is empty", () => {
    ...
    usernameSocket.verifyUsername(mockReq(request), mockRes(response));
    expect(usernameSocket.usernameArray.length).to.equal(0);
  });
  ...
  it("Should be true if multiple differents usernames are added", () => {
    ...
    usernameSocket.verifyUsername(mockReq(firstRequest), mockRes(response));
    usernameSocket.verifyUsername(mockReq(secondRequest), mockRes(response));
    usernameSocket.verifyUsername(mockReq(thirdRequest), mockRes(response));
    expect(usernameSocket.usernameArray.length).to.equal(3);
    expect(usernameSocket.usernameArray[0]).to.equal("username1");
    expect(usernameSocket.usernameArray[1]).to.equal("username2");
    expect(usernameSocket.usernameArray[2]).to.equal("username3");
    usernameSocket.deleteUsername(mockReq(firstRequest));
    usernameSocket.deleteUsername(mockReq(secondRequest));
    usernameSocket.deleteUsername(mockReq(thirdRequest));
  });
  it("Should be true if two identical usernames are added", () => {
    ...
    usernameSocket.verifyUsername(mockReq(request), mockRes(response));
    usernameSocket.verifyUsername(mockReq(request), mockRes(response));
    expect(usernameSocket.usernameArray.length).to.equal(1);
    expect(usernameSocket.usernameArray[0]).to.equal("username");
    usernameSocket.deleteUsername(mockReq(request));
  });
  // deleteUsername
